...
先看最重要的边界：本总仓当前没有三个运动应用的完整可构建源码，因此不能从这个总仓直接生成 Run、Bike 或 Rower AIX。要在 Craft 中运行，需要使用未来公开的独立应用源码仓库，或你自己有权使用的 AIUI 项目。
...
![源码、AIX、真机与平台的独立门](images/open-source-guide/06-release-gates.svg)
