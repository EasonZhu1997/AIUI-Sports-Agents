...
- **EverMind 对接边界**：提供面向长期记忆用途的客户端后端契约；是否由部署方接入 EverMind 属于仓库外责任，自建教练后端默认关闭
...
长期记忆是 AISmartRun 的可选网络契约，不是三套应用的共享运行时。默认沉浸页使用本地最近跑步生成固定跑后总结；兼容首页另有远端上下文请求路径。公开源码只包含客户端请求、队列、降级和测试；不包含 EverMind 服务、生产地址、密钥或后端实现，也不依赖 Raven，不能单凭客户端代码证明生产后端已接入 EverMind。
