...
function finishSession(session, end) {
  ...
  const rideId = String(begin.ride_id || '');
  ...
  if (!rideId || parts === null || bytes === null || !checksum) {
    errors.push('invalid_begin');
  }
  if (String(end.ride_id || '') !== rideId
  ...
  if (payloadText && cyclingLocalFieldLogUtf8Bytes(payloadText) !== bytes) {
    errors.push('byte_length_mismatch');
  }
  if (payloadText && cyclingLocalFieldLogChecksum(payloadText) !== checksum) {
  ...
  if (payload && String(payload.ride && payload.ride.ride_id || '') !== rideId) {
    errors.push('ride_id_mismatch');
    payload = null;
  }
  return {
    ok: errors.length === 0,
...
export function extractCyclingLocalFieldLogReplays(text) {
  const sessions = [];
  let active = null;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!active || String(value.ride_id || '')
        !== String(active.begin.ride_id || '')) continue;
...
function parseCli(argv) {
  ...
  if (!input) {
    throw new Error('Usage: node tools/extract_local_field_log.mjs <adb-log.txt> [--out result.json]');
  }
  return { input: path.resolve(input), output: output ? path.resolve(output) : '' };
}

export function runLocalFieldLogExtractor(argv = process.argv.slice(2)) {
  const options = parseCli(argv);
  const result = extractCyclingLocalFieldLogReplays(
...
if (process.argv[1]
  ...
  try { runLocalFieldLogExtractor(); } catch (error) {
