...
test('AIX 包内检查锁定默认沉浸首屏、兼容首页、多状态路由与停扫生命周期', () => {
  ...
  assert.match(source, /Settings visual order must be Stride, Voice, Metronome, Guide, Binding, passive EverMind, Heart, then the absolute Back control/);
  assert.match(source, /six 40px controls plus one 24px passive row, a 264px list, and a 24px footer inside the 480x352 canvas/);
  assert.match(source, /Settings must expose seven interactive targets with inward focus only on the selected target; the AI \/ EverMind row remains passive/);
  assert.match(source, /AI model and EverMind as non-interactive capability copy/);
  assert.match(source, /settings data layer must force AI summary and EverMind memory on/);
